...
\begin{lemma}
  ...
  \begin{proof}[Proof ($\approx${\cite[Lemma~5.12]{Morin-2014}})]
    We note that the result would follow from
    \begin{equation}
      \label{eqn:vanishing-of-Q/Z-cohomology}
      H^i (X_\et, \QQ/\ZZ^c (n)) = 0 \quad\text{for }i < -2\,\dim X.
    \end{equation}
    Similarly, if $X/\FF_q$ is a variety over a finite field, then we will prove
    this also for $i = -2\,\dim X$.

    Indeed, then the distinguished triangle
    ...
    Now we proceed to prove \eqref{eqn:vanishing-of-Q/Z-cohomology}.
...
\begin{remark}
  ...
  include the case of $i = -2d$, unless $X$ is a variety over a finite field.
\end{remark}
...
\begin{theorem}
  \label{thm:RGammaWc-perfect}
  The conjecture $\mathbf{L}^c (X_\et,n)$ implies that $R\Gamma_\Wc (X, \ZZ(n))$ is
  a perfect complex. Specifically,
  \begin{enumerate}
  ...
  \item[3)] if $H^{-2d} (X_\et, \ZZ^c(n)) = 0$, then
