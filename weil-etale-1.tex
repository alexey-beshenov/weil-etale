...
\begin{remark}
  The extra assumptions in 2) and 3) of the previous proposition can be seen
  already for varieties over finite fields $X/\FF_q$. As always assuming
  $\mathbf{L}^c (X_\et,n)$, the spectral sequence
  \eqref{eqn:RGamma-Wc-over-finite-fields-ss} shows that in this case
  \begin{align*}
    H^0_\Wc (X, \ZZ(n)) = 0 & \iff H^1 (X_\et, \ZZ^c (n))\text{ is finite}, \\
    H^{2d+2}_\Wc (X, \ZZ(n)) = 0 & \iff H^{-2d} (X_\et, \ZZ^c (n)) = 0.
  \end{align*}
  (For $X/\FF_q$ and $n < 0$, we in fact \emph{expect} $H^i (X_\et, \ZZ^c (n))$
  to be finite for all $i \in \ZZ$, at least for smooth proper $X$---see
  proposition~\ref{prop:Lc-Xet-n-over-finite-fields} below.)
\end{remark}
