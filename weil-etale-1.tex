...
\begin{lemma}[{cf. corollary~\ref{cor:RGamma-fg-model-for-l-adic-cohomology}}]
  ...
  \begin{proof}
    ...
    \[ \begin{tikzcd}[row sep=3em, column sep=3em]
        R\Gamma_c (X_\et, \ZZ (n)) \otimes_\ZZ^\mathbf{L} \ZZ/\ell^r \ar{r}\ar{d}\ar{dr}{u_\infty^* \otimes \ZZ/\ell^r} & R\widehat{\Gamma}_c (G_\RR, X(\CC), \ZZ(n)) \otimes_\ZZ^\mathbf{L} \ZZ/\ell^r\ar{d} \\
        R\Gamma_\fg (X, \ZZ(n)) \otimes_\ZZ^\mathbf{L} \ZZ/\ell^r \ar{r}{i_\infty^* \otimes \ZZ/\ell^r} & R\Gamma_c (G_\RR, X(\CC), \ZZ(n)) \otimes_\ZZ^\mathbf{L} \ZZ/\ell^r
      \end{tikzcd} \]
    The mapping fiber of the top horizontal arrow is the modified étale
    cohomology with compact support
    $R\widehat{\Gamma}_c (X_\et, \ZZ (n)) \otimes_\ZZ^\mathbf{L} \ZZ/\ell^r$
    (see e.g. \cite[Lemma~6.14]{Flach-Morin-2018}), while the mapping fiber of
    the bottom horizontal arrow is
    $R\Gamma_\Wc (X, \ZZ (n)) \otimes_\ZZ^\mathbf{L} \ZZ/\ell^r$. Therefore, we
    have a comparison morphism
    \begin{equation}
      \label{eqn:comparison-of-H-Wc-mod-lr}
      R\widehat{\Gamma}_c (X_\et, \ZZ (n)) \otimes_\ZZ^\mathbf{L} \ZZ/\ell^r \to
      R\Gamma_\Wc (X, \ZZ (n)) \otimes_\ZZ^\mathbf{L} \ZZ/\ell^r.
    \end{equation}
    We recall from proposition~\ref{prop:tensoring-RGammafg-with-Z/m-and-Q} that
    $R\Gamma_c (X_\et, \ZZ (n)) \otimes_\ZZ^\mathbf{L} \ZZ/\ell^r \to
    R\Gamma_\fg (X, \ZZ(n)) \otimes_\ZZ^\mathbf{L} \ZZ/\ell^r$ is an
    isomorphism. Similarly, the arrow
    $R\widehat{\Gamma}_c (G_\RR, X(\CC), \ZZ(n)) \to
    R\Gamma_c (G_\RR, X(\CC), \ZZ(n))$ induces isomorphisms in cohomology $H^i$
    for $i \ge 2d-1$ (see lemma~\ref{lemma:Tate-vs-normal-cohomology-of-X(C)}).
...
\begin{corollary}
  ...
  \begin{proof}
    ...
    \cite[Exposé~X, Théorème~6.2]{SGA4}\footnote{N.B. If $\ell = 2$ and
      ...
      modified cohomology for number fields \cite[Chapter~II]{Milne-ADT}.}.
  \end{proof}
\end{corollary}
...
\begin{remark}
  ...
  (cf. \cite[p.\,991]{Kahn-2003-equivalences} and proof of
