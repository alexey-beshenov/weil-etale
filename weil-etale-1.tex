...
and analytic topology on $X (\CC)$ identifies them.
...
Then \S\ref{sec:GR-equivariant-cohomology} is devoted to various lemmas about
$G_\RR$-equivariant cohomology of $X (\CC)$ that are needed in the subsequent
sections.

In \S\ref{sec:arithmetic-duality-theorem} we deduce from Geisser's results from
\cite{Geisser-2010}, a quasi-isomorphism of complexes
\[ R\widehat{\Gamma}_c (X_\et, \ZZ (n)) \cong
\RHom (R\Gamma (X_\et, \ZZ^c (n)), \QQ/\ZZ [-2]), \]
which is a sort of arithmetic duality theorem. It is then used in
\S\ref{sec:RGamma-fg} to define a morphism $\alpha_{X,n}$ in the derived
category, and declare $R\Gamma_\fg (X, \ZZ(n))$ to be its cone:
...
\begin{proposition}
  ...
  \begin{proof}
    ...
    First we observe that for $i \ll 0$ we have
...
$R\Gamma_\Wc (X, \ZZ(n))$. We begin with an auxiliary morphism. 
...
\begin{proof}[We are now ready to prove Theorem~\ref{thm:u-alpha-0}]
...
\begin{proposition}
  ...
  \begin{proof}
    ...
    Now if $j\colon X \hookrightarrow \mathfrak{X}$ is a Nagata
    compactification, we have the corresponding compactification
    ...
    $\mathcal{F}$ is a $G_\RR$-equivariant sheaf on $X (\CC)$, then $j (\CC)_!
    \mathcal{F}$ is a $G_\RR$-equivariant sheaf on $\mathfrak{X} (\CC)$ (this is
    evident from the definition of equivariant sheaves as equivariant espaces
    étalés). It should be clear from the definition of $\alpha^*$ that there is
