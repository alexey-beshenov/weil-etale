...
We first note that for varieties over finite fields, the finite generation
conjecture $\mathbf{L}^c (X_\et,n)$ implies \emph{finiteness} of motivic
cohomology.

\begin{proposition}
  \label{prop:finite-motivic-cohomology-for-X/Fq}
  Let $X/\FF_q$ be a variety over a finite field. Then $\mathbf{L}^c (X_\et,n)$
  for $n < 0$ implies that the groups $H^i (X_\et, \ZZ^c(n))$ are finite for all
  $i \in \ZZ$, and $H^i (X_\et, \ZZ^c (n)) = 0$ for $i > 1$ or $i < -2d$.

  \begin{proof}
    Our duality theorem~\ref{theorem-I} gives
    $$\Hom (H^{2-i} (X_\et, \ZZ^c (n)), \QQ/\ZZ) \cong H^i_c (X_\et, \ZZ (n)),$$
    and the cohomology groups on the right hand side are finite by
    \cite[Theorem~3]{Kahn-2003}. We have $H^i_c (X_\et, \ZZ (n)) = 0$ for
    $i < 1$ by the definition of $\ZZ (n)$, and $H^i_c (X_\et, \ZZ (n)) = 0$ for
    $i > 2\dim X + 2$ because of the cohomological dimension.
  \end{proof}
\end{proposition}

Similar boundedness of $H^i (X_\et, \ZZ^c (n))$ can be deduced for a general
arithmetic scheme.
...
\begin{proposition}
  ...
  \begin{proof}
    ...
    We recall from proposition~\ref{prop:finite-motivic-cohomology-for-X/Fq} that
    the groups $H^i (X_\et, \ZZ^c(n))$ are finite under our assumption.
