...
places of $X$. There is a canonical projection $R\widehat{\Gamma}_c (X_\et,
\mathcal{F}^\bullet) \to R\widehat{\Gamma} (X_\et, \mathcal{F}^\bullet)$, which
is an isomorphism whenever $X (\RR) = \emptyset$.
...
$R\Gamma_\fg (X, \ZZ(n))$ is almost of finite type in the sense of
Definition~\ref{dfn:almost-of-(co)finite-type}. Because of the specific
properties of the involved complexes, it turns out that
$R\Gamma_\fg (X, \ZZ(n))$ is defined up to a unique isomorphism in the derived
category (something one usually does not expect from a cone).

Finally, in \S\ref{sec:RGamma-Wc} we define Weil-étale complexes
$R\Gamma_\Wc (X, \ZZ(n))$. For this we firs consider a canonical morphism
...
\begin{equation}
  \label{eqn:diagram-for-RGammaWc}
...
The canonical morphism of complexes $\widehat{P}_\bullet \to P_\bullet$ gives a
canonical morphism
