...
\begin{proposition}
  ...
  \begin{proof}
    ...
    \[ \Hom (H^{2-i} (X_\et, \ZZ^c (n)), \QQ/\ZZ) \cong
      \widehat{H}^i_c (X_\et, \ZZ (n)) \stackrel{\text{if }X(\RR)=\emptyset}{\cong}
      H^i_c (X_\et, \ZZ (n)). \]
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    proposition~\ref{prop:motivic-cohomology-duality-consequences},
    and therefore $H^i_\fg (X_\et, \ZZ(n)) \cong H^i_c (X_\et, \ZZ(n))$.
...
\begin{proposition}
  ...
  \item[3)] if we further assume that $H^{-2d} (X_\et, \ZZ^c(n)) = 0$ or
    $X (\RR) = \emptyset$, then $H^{2d+2}_\Wc (X,\ZZ(n)) = 0$.
  \end{enumerate}
  ...
  \begin{proof}
    ...
    If $X (\RR) = \emptyset$, then by
    lemma~\ref{lemma:Tate-vs-normal-cohomology-of-X(C)} and
    \ref{lemma:morphism-hat-Hc(Xet,Z(n))->Hc(Xet,Z(n))} we have
    \[ \underbrace{H^{2d+1}_c (G_\RR, X (\CC), \ZZ(n))}_{= 0} \to \\
      H^{2d+2}_\Wc (X, \ZZ(n)) \to
      H^{2d+2}_\fg (X_\et, \ZZ(n)) \to
      \underbrace{H^{2d+2}_c (G_\RR, X (\CC), \ZZ(n))}_{= 0} \]
    so that $H^{2d+2}_\Wc (X, \ZZ(n)) \cong H^{2d+2}_\fg (X_\et, \ZZ(n))$.
    Now corollary~\ref{cor:RGamma-fg-model-for-l-adic-cohomology} gives for
    each $\ell$
    \[ H^{2d+2}_\Wc (X, \ZZ(n)) \otimes \ZZ_\ell \cong
      H_\fg^{2d+2} (X, \ZZ (n)) \otimes \ZZ_\ell \cong
      H^{2d+2}_c (X [1/\ell]_\et, \ZZ_\ell (n)) = 0 \]
    for the reasons of cohomological dimension
    \cite[Exposé~X, Théorème~6.2]{SGA4}. Since this holds for each $\ell$,
    we conclude that $H^{2d+2}_\Wc (X, \ZZ(n)) = 0$.
  \end{proof}
\end{proposition}
