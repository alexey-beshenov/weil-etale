...
\begin{abstract}
  ...
  $\Spec \ZZ$) of Krull dimension $1$. We write down a formula for the special
  value of $\zeta (X,s)$ at $s = n < 0$, in terms of \'{e}tale motivic
  cohomology of $X$ and a regulator. We prove it in the case when for each
  generic point $\eta \in X$ with $\fchar \kappa (\eta) = 0$, the extension
  $\kappa (\eta)/\QQ$ is abelian. Further, we conjecture that the formula holds
  ...
  calculate Weil-\'{e}tale cohomology of one-dimensional $X$ and make explicit
  the special value conjecture from \cite{Beshenov-Weil-etale-2} for this case.
\end{abstract}
...
Let $X$ be an \textbf{arithmetic scheme}, by which we mean throughout this
...
$\dim X = 1$, which is the case of interest in this article.
...
denote by $\mathcal{O}_F$ the corresponding ring of integers. Then
$\zeta_F (s) = \zeta (X,s)$ for $X = \Spec \mathcal{O}_F$ is the
\textbf{Dedekind zeta function} attached to $F$.  It is an easy consequence of
the well-known functional equation for $\zeta_F (s)$ that it has a zero at
...
$F$, and $R_F \in \RR$ is the regulator; see for instance
\cite[\S VII.5]{Neukirch-1999}.
...
terms of algebraic $K$-theory. Later these were also restated in terms of
the $p$-adic cohomology groups $H^i (\Spec \mathcal{O}_F [1/p]_\et, \ZZ_p (n))$
for $i = 1,2$ and all primes $p$; the corresponding formula is known as the
...
working with $p$-adic cohomology for varying $p$, it is more convenient for us
to use motivic cohomology.
...
We do not not make any attempt at giving a proper historical overview of the
subject and writing down all conjectural formulas; the interested reader may
consult, e.g. \cite{Kolster-2004,Goncharov-2005,Kahn-2005}.
...
that gives finitely generated abelian groups. Flach and Morin gave in
\cite{Flach-Morin-2018} a construction of Weil-\'{e}tale cohomology
groups $H^i_\Wc (X,\ZZ(n))$ for a proper and regular arithmetic scheme $X$, and
stated a precise conjectural relation of $H^i_\Wc (X,\ZZ(n))$ to
$\zeta^* (X,n)$.

In particular, in \cite[\S 5.8.3]{Flach-Morin-2018} they write down explicitly
their special value formula for the case of $X = \Spec \mathcal{O}_F$. For
$n \le 0$ and in terms of the cohomology groups $H^i (X_\et, \ZZ^c (n))$, it
reads
\begin{equation}
...
According to \cite[Proposition~5.35]{Flach-Morin-2018}, the formula
\eqref{eqn:flach-morin-zeta-F-formula} holds unconditionally for abelian number
fields $F/\QQ$, via reduction to the Tamagawa number conjecture of
...
$n > 0$ which involves a ``correction factor'' $C (X,n) \in \QQ$. In this text
we will not say anything about the case of $n > 0$; the reader may consult
\cite{Flach-Morin-2018} for further details, and also the subsequent papers
\cite{Flach-Morin-2020,Flach-Morin-2020-Muenster,Morin-2021-THH} which further
investigate the nature of the correction factor $C (X,n)$.
...
In order to state the main result, it is convenient to introduce the following
terminology.
...
\noindent (In particular, if $X$ lives entirely in positive characteristic, then
it is abelian.) The goal of this text is to prove the following result.
...
We further conjecture that the formula \eqref{eqn:special-value-formula} still
holds for non-abelian one-dimensional schemes. This is equivalent to the
Tamagawa number conjecture for non-abelian number fields.
...
cohomology in \S\ref{sec:Weil-etale-proof}. Namely, we observe in
\S\ref{sec:Weil-etale-proof} that the special value formula is the same as
conjecture $\mathbf{C} (X,n)$ that was stated in \cite{Beshenov-Weil-etale-2},
specialized to $\dim X = 1$ and written out more explicitly.
...
literature. Secondly, we go through the construction of Weil-\'{e}tale
cohomology $H^i_\Wc (X, \ZZ(n))$ from \cite{Beshenov-Weil-etale-1}, explaining
it in the case of one-dimensional schemes. It is not very surprising that such a
special value formula exists, but the right cohomological invariants to state it
were suggested by the Weil-\'{e}tale framework.
...
Note that for a finitely generated group $A$, the $\ZZ$-dual $A^*$ is free of
the same rank. If $A$ is finite, then there is a (non-canonical) isomorphism
$A^D \cong A$, and in particular $|A^D| = |A|$.

\paragraph{Schemes.}
Throughout this text $X$ always denotes a
\textbf{one-dimensional arithmetic scheme}; that is, separated scheme of finite
type $X \to \Spec \ZZ$ of Krull dimension $1$.

We remark that the restriction that $X$ is abelian
(Definition~\ref{dfn:abelian-scheme}) is only needed for the
...
\paragraph{Weights.}
The number $n$ will be a fixed \emph{strictly negative} integer.
...
Briefly, we let $\Delta^i = \Spec \ZZ [t_0,\ldots,t_i] / (1 - \sum_i t_i)$ be
the algebraic simplex. Denote by $z_n (X, i)$ the group freely generated by
algebraic cycles $Z \subset X \times \Delta^i$ of dimension $n+i$.
For $n < 0$ we consider the complex of \'{e}tale sheaves
...
is what we will call motivic cohomology throughout this text. For a proper
regular arithmetic scheme $X$ of pure dimension $d$ we have
$$\ZZ^c (n) \cong \ZZ (d-n) [2d],$$
where $\ZZ (m)$ is the other motivic complex that usually appears in the
literature.

For further results about $\ZZ^c (n)$, we refer to \cite{Geisser-2010}. In
particular, we recall from \cite[Corollary~7.2]{Geisser-2010} that the groups
$H^i (X_\et, \ZZ^c (n))$ satify the localization property: if $Z \subset X$ is a
...
arithmetic schemes $X$; see \S\ref{sec:etale-motivic-cohomology} below.

\subsection*{Outline of the paper}

In \S\ref{sec:devissage} we prove a d\'{e}vissage lemma, which shows how a
property that holds for curves over finite fields and for number rings may be
generalized to any one-dimensional arithmetic scheme. This is an elementary
argument, isolated in order not to repeat the same reasoning in several proofs.
...
well-known results in order to describe the motivic cohomology groups
$H^i (X_\et, \ZZ^c(n))$.

We define in \S\ref{sec:regulators} the regulator that appears in the special
value formula.

Our first ``elementary'' proof of the main result is contained in
\S\label{sec:direct-proof}.

Then \S\ref{sec:Weil-etale-cohomology-of-X} is dedicated to a calculation of the
Weil-\'{e}tale cohomology groups $H^i_\Wc (X, \ZZ(n))$ from
\cite{Beshenov-Weil-etale-1}, in the case of one-dimensional $X$. We use these
calculations in \S\ref{sec:Weil-etale-proof} in order to make explicit
Conjecture $\mathbf{C} (X,n)$ from \cite{Beshenov-Weil-etale-2}, again for
one-dimensional $X$. This gives a second, more conceptual proof of the main
result, and also explains how we arrived to \eqref{eqn:special-value-formula} in
the first place.

Finally, we conclude in \S\ref{sec:examples} with a couple of examples that
illustrate how the special value formula works.
...
\section{\'{E}tale motivic cohomology of one-dimensional $X$}
...
\begin{remark}
  ...
  usual Beilinson regulator map
  \[ H^1 (X_\et, \ZZ (1-n)) \to H^1_\mathcal{D} (G_\RR, X(\CC), \RR(1-n)) \]
  that appears in the special value conjecture in
...
We recall Conjecture $\mathbf{C} (X,n)$ from
...
\begin{example}
  ...
  see for instance Weibel's survey \cite{Weibel-2005}.
  % Strictly speaking, this reasoning is \emph{backward}: as explained in
  ...
  % well-known tables of $K$-groups of $\ZZ$.
  We conclude that
  \begin{align*}
