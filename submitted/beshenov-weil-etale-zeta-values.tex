...
\newcommand{\DB}{{\mathcal{D}\text{-\foreignlanguage{russian}{B}}}}
...
interpretation, related to \'{e}tale motivic cohomology of $X$
(see e.g. \cite{Lichtenbaum-1984} for varieties over finite fields).
This is made more precise in Lichtenbaum's \textbf{Weil-\'{e}tale program}.
In particular it suggests the existence of a cohomology theory
$H^i_\Wc (X, \ZZ (n))$, \textbf{Weil-\'{e}tale cohomology} with compact support,
...
corresponding Weil-\'{e}tale topos. Later, Morin gave in \cite{Morin-2014} an
explicit construction of Weil-\'{e}tale cohomology groups $H^i_\Wc (X, \ZZ (n))$ for
$n = 0$, and $X$ a proper, regular arithmetic scheme, under assumptions on
finite generation of suitable \'{e}tale motivic cohomology groups. This construction
...
\cite{Beshenov-Weil-etale-1} the Weil-\'{e}tale cohomology groups
...
following assumption (see \cite[\S 8]{Beshenov-Weil-etale-1} for the known
cases).
...
Weil-\'{e}tale cohomology to the special value of $\zeta (X,s)$ at $s = n < 0$.
...
\begin{maintheorem*}
  ...
  $X_{\red,\CC}$, then Conjecture~$\mathbf{VO} (X,n)$ and
...
\cite{Kerr-Lewis-Muller-Stach-2006}, and state Conjecture~$\mathbf{B} (X,n)$
related to it.
...
is consistent with a conjecture of Soul\'{e} and the vanishing orders that come
from the expected functional equation
...
finite field, Conjecture~$\mathbf{C} (X,n)$ is consistent with the conjectures
considered by Geisser in
\cite{Geisser-2004,Geisser-2006,Geisser-2010-arithmetic-homology}.
In particular, it holds for any smooth projective $X/\FF_q$, assuming
...
complexes constructed in \cite{Beshenov-Weil-etale-1}, assuming
Conjecture~$\mathbf{L}^c (X_\et,n)$ stated above. For $\QQ$-coefficients, we put
\begin{align*}
  R\Gamma_\fg (X, \QQ (n)) & \dfn
  R\Gamma_\fg (X, \ZZ (n)) \otimes^\mathbf{L} \QQ =
  R\Gamma_\fg (X, \ZZ (n)) \otimes \QQ, \\
  R\Gamma_\Wc (X, \QQ (n)) & \dfn
  R\Gamma_\Wc (X, \ZZ (n)) \otimes^\mathbf{L} \QQ =
  R\Gamma_\Wc (X, \ZZ (n)) \otimes \QQ.
\end{align*}
Accordingly,
\begin{align*}
  H^i_\fg (X, \QQ (n)) & \dfn
  H^i (R\Gamma_\fg (X, \QQ (n))) = H^i_\fg (X, \ZZ (n)) \otimes \QQ, \\
  H^i_\Wc (X, \QQ (n)) & \dfn
  H^i (R\Gamma_\Wc (X, \QQ (n))) = H^i_\Wc (X, \ZZ (n)) \otimes \QQ.
\end{align*}
...
Universit\'{e} de Bordeaux and Universiteit Leiden under supervision of Baptiste
...
many useful comments and suggestions. Finally, I thank
Jos\'{e} Jaime Hern\'{a}ndez Castillo, Diosel L\'{o}pez Cruz, and Maxim Mornev
for various fruitful conversations.

This paper was edited while I was visiting Center for Research in Mathematics
(CIMAT), Guanajuato. I am grateful personally to Pedro Luis del \'{A}ngel and
...
to use the construction of Kerr, Lewis, and M\"{u}ller-Stach from
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    \end{align*}
    Here the last isomorphism is Poincar\'{e} duality.
  \end{proof}
\end{lemma}
...
\begin{remark}
  ...
  morphism or Conjecture~$\mathbf{B} (X,n)$ is brought, we will tacitly
  assume this. This is quite unfortunate, because Weil-\'{e}tale cohomology was
  constructed in \cite{Beshenov-Weil-etale-1} for any arithmetic scheme,
  assuming only $\mathbf{L}^c (X_\et,n)$. Defining the regulator for singular
  $X_{\red,\CC}$ is an interesting project for further work.
\end{remark}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Conjecture $\mathbf{VO} (X,n)$ (vanishing order)}
...
We note that the right hand side makes sense assuming
Conjecture~$\mathbf{L}^c (X_\et,n)$, under which $H^i_\Wc (X, \ZZ (n))$ are
finitely generated groups, trivial for $|i| \gg 0$ (see
\cite[Proposition~7.12]{Beshenov-Weil-etale-1}).
...
\begin{remark}
  ...
  \textbf{secondary Euler characteristic} of the Weil-\'{e}tale complex
...
Under the regulator conjecture, our vanishing order formula takes form of the
usual Euler characteristic of the equivariant cohomology
...
\begin{proposition}
  ...
  \begin{proof}
    Thanks to \cite[Proposition~7.14]{Beshenov-Weil-etale-1}, the Weil-\'{e}tale
...
\section{Conjecture~$\mathbf{C} (X,n)$ (special value)}
...
We recall that the Weil-\'{e}tale complex $R\Gamma_\Wc (X, \ZZ(n))$ was defined in
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    \end{multline*}
    tensored with $\QQ$ (see \cite[\S 5, **TODO** BE MORE SPECIFIC]{Beshenov-Weil-etale-1}).
...
\begin{remark}
  ...
  \eqref{eqn:FM-special-value} coincides with Conjecture~$\mathbf{C} (X,n)$
  for $n < 0$.
...
compatibility. We briefly recall the construction of our Weil-\'{e}tale
complex. It fits in the following diagram in the derived category
...
\begin{remark}
  We recall that taking naively a ``cone of a morphism of distinguished
  triangles''
