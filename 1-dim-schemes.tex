...
\begin{equation}
  ...
  \quad \text{for }n \le 0.
\end{equation}
The definitions of groups $H^i (X_\et, \ZZ^c (n))$ and the regulator
$R_{F,n} = R_{\Spec \mathcal{O}_F,n}$ will be given below. According to
\cite[Proposition~5.35]{Flach-Morin-2018}, the above formula holds
unconditionally for abelian number fields $F/\QQ$, via reduction to the Tamagawa
...
\begin{theorem}
  ...
  \item $R_{X,n}$ is a positive real number, defined below via Beilinson's
    regulator map.
  \end{itemize}
\end{theorem}
...
We will give two proofs of \eqref{eqn:flach-morin-zeta-F-formula}. The first
proof can be found in \S\ref{sec:special-value-formula} after some preliminary
...
\begin{definition}
  ...
  via
  $$R_{X,n} \dfn \vol (\coker \varrho^B_n),$$
  where
  \[ \varrho^B_n\colon
    H^{-1} (X_\et, \ZZ^c (n)) \to H^0_\BM (G_\RR, X(\CC), \RR(n)). \]
  is Beilinson's regulator map.
\end{definition}

The right hand side is Borel--Moore cohomology, defined via
\[ H^0_\BM (G_\RR, X(\CC), \RR(n)) \cong
...
We note that in particular, if $X = \Spec \mathcal{O}_F$, then
$\varrho^B_n$ identifies with the map
$H^1 (X_\et, \ZZ (1-n)) \to H^1_\BM (G_\RR, X(\CC), \RR(1-n))$
that appears in the special value conjecture in
\cite[\S 5.8.3]{Flach-Morin-2018}.

Tensored with $\RR$, the regulator becomes an isomorphism. In particular, as
will be explained below, we have
\[ \rk_\ZZ H^{-1} (X_\et, \ZZ^c (n)) =
  \rk_\ZZ H^0_c (G_\RR, X(\CC), \ZZ(n)) = d_n. \]
...
one-dimensional $X$. In \S\ref{sec:special-value-formula} these calculations are
used to write down an explicit conjectural formula for the special value
$\zeta^* (X,n)$, which corresponds to the conjecture $\mathbf{C} (X,n)$ from
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    \item If $X \to \Spec \ZZ$ is a dominant morphism, consider an open affine
      ...
      the same claim for $\Spec \mathcal{O}_F$. \qedhere
    \end{itemize}
  \end{proof}
\end{lemma}
...
stated in \cite[\S 4]{Beshenov-Weil-etale-2}. We are interested in the
determinant of Weil-étale complex
\begin{multline*}
...
\end{multline*}
Recall that $T_1$, $T_2$, $H^3_\Wc (X, \ZZ(n))^{-1}$ are finite groups, and
$H^0_c (G_\RR, X (\CC), \ZZ(n))^{-1}$ and $\Hom (H^{-1} (X_\et, \ZZ^c(n)), \ZZ)$
...
\[ \begin{tikzcd}[column sep=3em]
    H^1_\Wc (X, \ZZ(n)) \otimes \RR\ar{d}{\cong} & H^2_\Wc (X, \ZZ(n)) \otimes \RR\ar{d}{\cong} \\
...
Some linear algebra\footnote{Namely, \cite[Lemma~A.6]{Beshenov-Weil-etale-2} for
  finite groups $T_1$, $T_2$, $H^3_\Wc (X, \ZZ(n))^{-1}$. Further, the explicit
  description of trivialization from \cite[p.\,33]{Knudsen-Mumford-1976} shows
  that if $F$ and $F'$ are free groups of finite rank $d$, and
  $$C^\bullet\colon 0 \to F\otimes_\ZZ \RR \xrightarrow{\phi} F'\otimes_\ZZ \RR \to 0$$
  is a two-term acyclic complex of real vector spaces, then
  $\det_\ZZ F \otimes_\ZZ (\det_\ZZ F')^{-1} \subset \det_\RR (C^\bullet) \cong \RR$
  corresponds to $D\ZZ \subset \RR$, where $D$ is the determinant of $\phi$ with
  respect to the bases induced by $\ZZ$-bases of $F$ and $F'$.}  shows that
under this trivialization, $\det_\ZZ R\Gamma_\Wc (X, \ZZ(n)) \subset \RR$
corresponds to $\frac{1}{m}\,\ZZ \subset \RR$, where
\[ m = \frac{|T_2|}{|T_1| \cdot |R\Gamma_\Wc (X, \ZZ(n))|}\,R_{X,n}
...
We recall that conjecture $\mathbf{C} (X,n)$ from \cite{Beshenov-Weil-etale-2}
asserts that $\det_\ZZ R\Gamma_\Wc (X, \ZZ(n)) \subset \RR$ corresponds to
...
\section{Demystification of the formula}
...
$X = \Spec \mathcal{O}_F$ and curves over finite fields $X/\FF_q$. This in a
sense will demystify our result, but the two previous sections also serve their
purpose: these explain how we came up with the formula
\eqref{eqn:special-value-formula} in the first place.
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    For the last claim, note that $X (\CC) = U (\CC)$, and from the long exact
    sequence \eqref{eqn:localization-les}, we have
    $H^{-1} (X_\et, \ZZ^c (n)) \cong H^{-1} (U_\et, \ZZ^c (n))$, which means
    that the change in the integral structure is irrelevant for the volume.
