...
\begin{itemize}
\item \textbf{Step 1}. Consider the morphism of complexes
...
\item \textbf{Step 2}. One considers a canonical morphism in the derived
  category $i^*_\infty$ that is torsion and gives a commutative diagram
  \[ \begin{tikzcd}
...
\begin{proposition}
  ...
  \item[2)] There is an isomorphism of finitely generated groups
    \[ H^2_\Wc (X, \ZZ(n)) \cong
      ...
      \oplus
      \Hom (\underbrace{H^0 (X_\et, \ZZ^c(n))}_{\text{finite}}, \QQ/\ZZ). \]

  \item[3)] There is an isomorphism of finite groups
    $$H^3_\Wc (X, \ZZ(n)) \cong \Hom (H^{-1} (X_\et, \ZZ^c(n))_\tors, \QQ/\ZZ).$$
  \end{enumerate}
  ...
  \begin{proof}
    ...
    \begin{align*}
      \ker H^i (\widehat{\alpha}_{X,n}) & = \Hom (H^{2-i} (X_\et, \ZZ^c(n)), \ZZ), \\
      \coker H^i (\widehat{\alpha}_{X,n}) & \cong \widehat{H}^i_c (X_\et, \ZZ(n))_\codiv.
    \end{align*}
    ...
    \begin{equation}
      \label{eqn:hat-fg-cohomology-calculation}
      \widehat{H}^i_\fg (X, \ZZ(n)) \cong
      \Hom (H^{1-i} (X_\et, \ZZ^c(n)), \ZZ)
      \oplus
      \widehat{H}^i_c (X_\et, \ZZ(n))_\codiv.
    \end{equation}
    ...
    \begin{align*}
      ...
      \tag{by \eqref{eqn:hat-fg-cohomology-calculation}}
                          & \cong \Hom (H^{1-i} (X_\et, \ZZ^c(n)), \ZZ)
                            \oplus
                            \widehat{H}^i_c (X_\et, \ZZ(n))_\codiv \\
      \tag{by duality \eqref{eqn:arithmetic-duality}}
                          & \cong \Hom (H^{1-i} (X_\et, \ZZ^c(n)), \ZZ)
                            \oplus
                            \Hom (H^{2-i} (X_\et, \ZZ^c (n))_\tors, \QQ/\ZZ).
    \end{align*}
    Our calculations of motivic cohomology show that $H^i_\Wc (X, \ZZ(n)) = 0$
    for $i \ge 4$, and
    \begin{align*}
      H^2_\Wc (X, \ZZ(n)) & \cong \Hom (H^{-1} (X_\et, \ZZ^c(n)), \ZZ)
                            \oplus
                            \Hom (H^0 (X_\et, \ZZ^c (n)), \QQ/\ZZ), \\
      H^3_\Wc (X, \ZZ(n)) & \cong \Hom (H^{-1} (X_\et, \ZZ^c (n))_\tors, \QQ/\ZZ). \qedhere
    \end{align*}
  \end{proof}
\end{proposition}
...
\begin{remark}
  ...
  $H^i_\Wc (X, \ZZ(n)) \cong \Hom (H^{2-i} (X_\et, \ZZ^c(n)), \QQ/\ZZ)$.  This
  holds for any variety over a finite field $X/\FF_q$, assuming finite
  generation of motivic cohomology $H^i (X_\et, \ZZ^c(n))$;
...
\begin{align*}
  ...
  \det_\ZZ H^2_\Wc (X, \ZZ(n)) & \cong \det_\ZZ \Hom (H^{-1} (X_\et, \ZZ^c(n)), \ZZ) \otimes_\ZZ \det_\ZZ \Hom (H^0 (X_\et, \ZZ^c(n)), \QQ/\ZZ).
\end{align*}
...
\begin{multline*}
  ...
  \det_\ZZ (T_1)^{-1} \det_\ZZ  \det_\ZZ \Hom (H^0 (X_\et, \ZZ^c(n)), \QQ/\ZZ) \otimes_\ZZ \det_\ZZ H^3_\Wc (X, \ZZ(n))^{-1}.
\end{multline*}
Recall that $T_1$, $\Hom (H^0 (X_\et, \ZZ^c(n)), \QQ/\ZZ)$,
$H^3_\Wc (X, \ZZ(n))$ are finite groups, while the groups
$H^0_c (G_\RR, X(\CC), \ZZ(n))$ and $\Hom (H^{-1} (X_\et, \ZZ^c(n)), \ZZ)$ are free
of rank $d_n$. Now we consider the canonical trivialization
...
\begin{proposition}
  ...
  \begin{align*}
    \alpha & = \frac{|\Hom (H^0 (X_\et, \ZZ^c(n)), \QQ/\ZZ)|}{|T_1| \cdot |\Hom (H^{-1} (X_\et, \ZZ^c(n))_\tors, \QQ/\ZZ)|}\,R_{X,n} \\
  ...
  \begin{proof}
    For finite groups $T_1$, $\Hom (H^0 (X_\et, \ZZ^c(n)), \QQ/\ZZ)$,
    $\Hom (H^{-1} (X_\et, \ZZ^c(n))_\tors, \QQ/\ZZ)$, this is
    \cite[Lemma~A.6]{Beshenov-Weil-etale-2}. On the other hand, for free groups
    $H^0_c (G_\RR, X(\CC), \ZZ(n))$ and $\Hom (H^{-1} (X_\et, \ZZ^c (n)), \ZZ)$,
