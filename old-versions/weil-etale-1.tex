...
\begin{abstract}
  ...
  consider the definition and basic properties of Weil-\'{e}tale cohomology.
  The second part will deal with the relation of $H^i_\Wc (X, \ZZ(n))$ with the
  special value of zeta function $\zeta (X,s)$ at $s = n < 0$.
\end{abstract}
...
$R\Gamma_\Wc (X, \ZZ (n))$. Their relation to the special value of $\zeta (X,s)$
at $s = n$ will be treated in the forthcoming Part~II.
...
\begin{center}
  ...
  \begin{tabular}{cc}
    ...
    \hline
    $R\Gamma_\fg (X, \ZZ(n))$ & \begin{tabular}{c} $R\Gamma_W (\overline{X}, \ZZ(n))$, \\ up to finite $2$-torsion \end{tabular} \\
    \hline
...
work on Weil-étale cohomology for varieties over finite fields
...
Ángel and Xavier Gómez Mont for their hospitality.
...
$H^i_c (X (\CC), \ZZ)$ are finitely generated and zero for $|i| \gg 0$.  This
follows from the fact that $X (\CC)$ has homotopy type of a finite CW-complex
\cite{van-der-Waerden-30}. The same is true for $H^i_c (X (\CC), \ZZ (n))$ (here
the twist in $\ZZ (n)$ only changes the $G_\RR$-module structure). For $\QQ/\ZZ$
coefficients, the following result will be useful.
...
---see for instance \cite[\S 1.6]{Krause-2007}.
