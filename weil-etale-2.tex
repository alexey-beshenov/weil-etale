...
\begin{proposition}
  ...
  \begin{proof}
    ...
    $\zeta (B,s)$, motivic cohomology $H^i (B_\et, \ZZ^c (n))$, or Weil-étale
    cohomology $H^i_\Wc (B, \ZZ(n))$, so we may assume that $B$ is reduced.

    Consider the normalization $f\colon B' \to B$. Since we are in dimension
    $1$, the scheme $B'$ is regular, and we have a cartesian square
    \[ \begin{tikzcd}
