...
\begin{maintheorem*}
  ...
  fiber $X_{\red,\CC}$, then the conjectures $\mathbf{VO} (X,n)$ and
...
\begin{remark}
  ...
  cycle complexes / higher Chow groups, and these fail to distinguish $X$ from
...
\begin{lemma}
  ...
  \begin{proof}
    Thanks to $\mathcal{C}0)$, we may assume that $X$ is reduced. In this case
    $X$ is a disjoint union of $\Spec \FF_q$ for finite fields.
  \end{proof}
\end{lemma}
...
\begin{proposition}
  ...
  \begin{proof}
    ...
    $C/\FF_q$, using the operations $\mathcal{C}0)$--$\mathcal{C}3)$ that appear
    in the definition of $\mathcal{C} (\ZZ)$.

    Thanks to $\mathcal{C}0)$, we may assume without loss of generality that $B$
    is reduced. $B$ has finitely many irreducible components, corresponding to
    generic points $\eta_1,\ldots,\eta_m$. Let $U_1,\ldots,U_m$ be disjoint open
    affines containing these points. Then $B\setminus U$ for $U = \bigcup_i U_i$
    has dimension $0$, so it lies in $\mathcal{C} (\ZZ)$. Using the operation
    $\mathcal{C}2)$, we see that it is enough to prove that
    $U \in \mathcal{C} (\ZZ)$, i.e. we may also assume that $B$ itself is
    affine.
    ...
    \[ \begin{tikzcd}
      ...
      \end{tikzcd} \]
    where $Z \subset B$ is the $0$-dimensional non-regular locus,
    $Z' \subset B'$ is $0$-dimensional as well, so that
    $Z, Z' \in \mathcal{C} (\ZZ)$. Now $f$ induces an isomorphism
    $B\setminus Z \cong B'\setminus Z'$, and it will
    be enough to see that $B' \in \mathcal{C} (\ZZ)$, and then
    \begin{align*}
    ...
    If $C_i \subset B$ are the irreducible components of $B$, then
    $B' = \coprod_i C_i'$ is the disjoint union of normalizations, so we may
    assume that $B'$ is irreducible thanks to $\mathcal{C}1)$.

    Now there are two cases. If the image of $B' \to \Spec \ZZ$ is a closed
    point, then $B'$ is a curve over $\FF_p$, so that
    $B' \in \mathcal{C} (\ZZ)$.  On the other hand, if $B' \to \Spec \ZZ$ is a
    dominant morphism, then for the generic point $\eta \in B'$ the residue
    field $F = \kappa (\eta)$ is a finite abelian extension of $\QQ$ by our
    assumptions. We have $B' = \Spec \mathcal{O}$, where
    $\mathcal{O}_F \subseteq \mathcal{O} = \mathcal{O}_{F,S}$ for some finite
    set $S$, since $\mathcal{O}$ is an integrally closed finitely generated
    domain. Now $B' = \Spec \mathcal{O}_F \setminus S$ and
    $S \in \mathcal{C} (\ZZ)$, so again, everything reduces to the case of
    $B' = \Spec \mathcal{O}_F$, which lies in $\mathcal{C} (\ZZ)$ by the
    definition.
  \end{proof}
\end{proposition}
...
\begin{remark}
  ...
  formula at $s = 1$, generalizing the classical class number formula.  They
  consider the case of $B$ reduced and affine, albeit without requiring
