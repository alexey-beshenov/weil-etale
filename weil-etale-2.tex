...
\begin{maintheorem*}
  Let $B$ be an affine arithmetic scheme of pure dimension $1$, such that each
  of the generic points $\eta \in X$ satisfies one of the following properties:
  \begin{enumerate}
  ...
  \item[b)] $\fchar \kappa (\eta) = 0$, and $\kappa (\eta)/\QQ$ is an abelian
    number field.
  \end{enumerate}
  If $X$ is a $B$-cellular arithmetic scheme $X$ with smooth quasi-projective
  fiber $X_\CC$, then $\mathbf{C} (X,n)$ holds unconditionally for any $n < 0$.
\end{maintheorem*}
...
$X_\CC$ is a smooth quasi-projective variety.
...
\begin{remark}[{\cite[Proposition~5.12]{Flach-Morin-2018}}]
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    \cite[Proposition~5.5]{Beshenov-Weil-etale-1}, these cones are well-defined
    ...
    (see \cite[Corollary~A.3]{Beshenov-Weil-etale-1}). It does not have to be a
    distinguished triangle\footnote{Taking naively a ``cone of a morphism of
      distinguished triangles''
      ...
      issues associated to working with the classical derived categories.},
...
\begin{remark}
  ...
  we apply it to the canonical closed embedding $X^{red} \hookrightarrow X$,
  then we conclude that
  $R\Gamma_\Wc (X, \ZZ(n)) \cong R\Gamma_\Wc (X^{red}, \ZZ(n))$. This is not
...
\begin{proposition}
  ...
  Let $B$ be an affine arithmetic scheme of dimension $1$. Assume that each of
  the generic points $\eta \in B$ satisfies one of the following properties:
  \begin{enumerate}
  \item[a)] $\fchar \kappa (\eta) = p > 0$;
  ...
  \item[b)] $\fchar \kappa (\eta) = 0$, and $\kappa (\eta)/\QQ$ is an abelian
    number field.
  \end{enumerate}
  Then the conjecture $\mathbf{C} (B,n)$ holds for any $n < 0$.

  \begin{proof}
    Passing from $B$ to $B^\mathrm{red}$ does not affect the zeta function
    $\zeta (B,s)$ or Weil-étale cohomology $H^i_\Wc (B, \ZZ(n))$, so we may
    assume that $B$ is reduced. We furter consider the normalization
    $f\colon B' \to B$. Since we are in dimension $1$, the scheme $B'$ is
    regular, and we have a cartesian square
    \[ \begin{tikzcd}
        Z' \ar[right hook->]{r}{i'}\tikzpb\ar{d}{f'} & B' \ar{d}{f} \\
        Z \ar[right hook->]{r}{i} & B
      \end{tikzcd} \]
    where $Z \subset B$ is the finite non-regular locus, $Z' \subset B'$ is
    finite as well, and $f$ induces an isomorphism
    $B\setminus Z \cong B'\setminus Z'$.

    The special value conjecture is true for $0$-dimensional schemes
    (see example~\ref{example:C(X,n)-for-Spec-Fq}), so $\mathbf{C} (Z,n)$ and
    $\mathbf{C} (Z',n)$ hold. Now if we establish $\mathbf{C} (B',n)$, then
    $\mathbf{C} (B,n)$ would follow by compatibility with closed-open
    decompositions (theorem~\ref{thm:compatibility-of-C(X,n)}):
    \begin{align*}
      \mathbf{C} (B',n) + \mathbf{C} (Z',n) & \Longrightarrow \mathbf{C} (B'\setminus Z', n) \iff \mathbf{C} (B\setminus Z, n),\\
      \mathbf{C} (B\setminus Z, n) + \mathbf{C} (Z,n) & \Longrightarrow \mathbf{C} (B,n).
    \end{align*}

    The special value conjecture is also compatible with disjoint unions of
    schemes, so we may assume that $B' = \Spec \mathcal{O}_{F,S}$, where $F$ is
    a global field, and $\mathcal{O}_{F,S}$ is the ring of $S$-integers
    (see e.g. \cite[\S\S 3,4]{Jordan-Poonen-2020}). Here
    $\Spec \mathcal{O}_{F,S} = \Spec \mathcal{O}_F \setminus S$, where $S$ is
    finite, so again, thanks to the compatibility with closed-open
    decompositions, it will be enough to consider the case of
    $B' = \Spec \mathcal{O}_F$.

    If $\fchar F = p > 0$, then $\mathbf{C} (B',n)$ holds for any $n < 0$ thanks
    to theorem~\ref{thm:C(X,n)-over-finite-fields}. The only detail to verify is
    that the cohomology groups $H^i (B'_\et, \ZZ^c (n))$ are finite. This
    essentially goes back to Soulé;
    see e.g. \cite[Proposition~4.3]{Geisser-2017}\footnote{The quoted result
      states that $H^i (B_\et, \ZZ^c (n)) = H^{i+2} (B_\et, \ZZ (1-n))$ is
      finite, except for $i = -2n, -2n + 2$, when it is possibly finitely
      generated or of cofinite type. However, Artin--Verdier duality
      \cite[Theorem~I]{Beshenov-Weil-etale-1} shows that these two
      ``exceptional'' groups are at most finite $2$-torsion.}.

    Now if $\fchar F = 0$, then by our assumption, $F/\QQ$ is an abelian number
    field. In this case the conjecture $\mathbf{C} (B',n)$ is equivalent to
...
\begin{remark}
  The schemes as above were considered in \cite{Jordan-Poonen-2020}, where the
  authors write down a special value formula at $s = 1$, generalizing the
  classical class number formula.
\end{remark}
...
\begin{definition}
  ...
  \begin{equation}
    ...
    X = X_N \supseteq X_{N-1} \supseteq \cdots \supseteq X_0 \supseteq X_{-1} = \emptyset
  \end{equation}
...
$\Gr (k,\ell)_B$ are cellular.

\begin{theorem}
  Let $B$ be an affine $1$-dimensional arithmetic scheme satisfying the
  assumptions of proposition~\ref{prop:particular-cases-1-dim-base}.
  If $X$ is a $B$-cellular arithmetic scheme $X$ with smooth and
  quasi-projective fiber $X_\CC$, then the special value $\mathbf{C} (X,n)$
  holds unconditionally for any $n < 0$.

  \begin{proof}
    The conjecture $\mathbf{C} (X,n)$ makes sense only if the regulator map is
    defined, for which we need to assume that $X_\CC$ is smooth and
    quasi-projective
    (see remark~\ref{rmk:regulator-is-defined-for-XC-smooth-quasi-proj}).
    Now looking at the corresponding cellular decomposition
    ...
    and the regulator morphisms make sense. Using lemma
    \ref{lemma:compatibility-of-B(X,n)}, we deduce $\mathbf{B} (X,n)$
    by induction on the length of the cellular decomposition.

    By a similar inductive argument, we deduce $\mathbf{L}^c (X_\et, n)$.
    Finally, the conjecture $\mathbf{C} (B,n)$ holds for all $n < 0$ thanks to
    proposition~\ref{prop:particular-cases-1-dim-base}, so the induction implies
    $\mathbf{C} (X,n)$ for all $n < 0$, using compatibility of
    $\mathbf{C} (-,n)$ with closed-open decompositions, affine bundles, and
    disjoint unions (theorem~\ref{thm:compatibility-of-C(X,n)}).
  \end{proof}
\end{theorem}
