...
\begin{maintheorem*}
  ...
  fiber $X_\CC$, then the conjectures $\mathbf{VO} (X,n)$ and $\mathbf{C} (X,n)$
  hold unconditionally for any $n < 0$.
\end{maintheorem*}
...
\begin{definition}
  ...
  \item[$\mathcal{C}2)$] Let $Z \not\hookrightarrow X \hookleftarrow U$ be a
    closed-open decomposition such that $Z_\CC, X_\CC, U_\CC$ are smooth and
...
We recall that the condition that $X_\CC$ is smooth and quasi-projective is
needed to ensure that the regulator morphism exists
(see remark~\ref{rmk:regulator-is-defined-for-XC-smooth-quasi-proj}).
Also note that it is a formal consequence of $\mathcal{C}2)$ that
$X \in \mathcal{C} (\ZZ)$ if and only if $X^{red} \in \mathcal{C} (\ZZ)$.
...
\begin{proposition}
  ...
  \begin{proof}
    ...
    \cite[Proposition~4.3]{Geisser-2017}\footnote{Assuming $X$ is smooth,
      ...
      ``exceptional'' groups are at most finite $2$-torsion. If $X$ is not
    ...
    the operations $\mathcal{C}1)$, $\mathcal{C}2)$, $\mathcal{C}3)$ is
...
\begin{proposition}
  ...
  \begin{proof}
    We will see that such a scheme may me obtained from $\Spec \mathcal{O}_F$
    for an abelian number field $F/\QQ$, or a curve over a finite field
    $C/\FF_q$, using the operations $\mathcal{C}1)$--$\mathcal{C}3)$ that appear
    in the definition of $\mathcal{C} (\ZZ)$.

    We may pass from $B$ to $B^\mathrm{red}$. We further consider the
    normalization $f\colon B' \to B$. Since we are in dimension $1$, the scheme
    $B'$ is regular, and we have a Cartesian square
    \[ \begin{tikzcd}
        Z' \ar[right hook->]{r}{i'}\ar{d}{f'} & B' \ar{d}{f} \\
    ...
    Since finite fields and their unions are in $\mathcal{C} (\ZZ)$, we have
    $Z, Z' \in \mathcal{C} (\ZZ)$. It will be enough to see that
    $B' \in \mathcal{C} (\ZZ)$, and then
    \begin{align*}
      B', Z' \in \mathcal{C} (\ZZ) & \Longrightarrow B'\setminus Z' \in \mathcal{C} (\ZZ) \iff B\setminus Z \in \mathcal{C} (\ZZ),\\
      B\setminus Z, \, Z \in \mathcal{C} (\ZZ) & \Longrightarrow B \in \mathcal{C} (\ZZ).
    \end{align*}

    Since $\mathcal{C} (\ZZ)$ is closed under disjoint unions of schemes, we may
    assume that $B' = \Spec \mathcal{O}_{F,S}$, where $F$ is a global field, and
    $\mathcal{O}_{F,S}$ is the ring of $S$-integers
    (see e.g. \cite[\S\S 3,4]{Jordan-Poonen-2020}).
    Here $\Spec \mathcal{O}_{F,S} = \Spec \mathcal{O}_F \setminus S$, where $S$
    is finite, so it will be enough to consider the case of
    $B' = \Spec \mathcal{O}_F$. By our assumptions on $B$, this is either an
    affine curve over a finite field, or spectrum of the ring of integers of an
    abelian number field, and these lie in $\mathcal{C} (\ZZ)$ by the definition.
  \end{proof}
\end{proposition}
...
\begin{proposition}
  ...
  and $X_\CC$ is smooth and quasi-projective. Then $X \in \mathcal{C} (\ZZ)$.
...
\begin{theorem}
  ...
  quasi-projective fiber $X_\CC$, then the conjectures $\mathbf{VO} (X,n)$ and
