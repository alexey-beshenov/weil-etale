...
\begin{proposition}
  ...
  \begin{proof}
    ...
    is reduced. $B$ is a finite union of its irreducible components, so thanks
    to $\mathcal{C}1)$ we may also assume that $B$ is irreducible. Further, if
    $U \subset B$ is an affine neighborhood of the generic point, then
    $B \setminus U$ is finite, so it lies in $\mathcal{C} (\ZZ)$. Using the
    operation $\mathcal{C}2)$, we see that it is enough to prove that
    ...
    domain. Now $B' = \Spec \mathcal{O}_F \setminus S$, where $S$ is finite, so
    again, everything reduces to the case of $B' = \Spec \mathcal{O}_F$, which
    lies in $\mathcal{C} (\ZZ)$ by the definition.
  \end{proof}
\end{proposition}
