...
\begin{lemma}
  ...
  \begin{proof}
    Since $X$ is a noetherian scheme of dimension $0$, it is a disjoint union of
    $\Spec A_i$ for some artinian local rings $A_i$. Thanks to $\mathcal{C}1)$,
    we may assume that $X = \Spec A$, and thanks to $\mathcal{C}0)$, we may
    assume that $X$ is reduced. But then $A = k$ is a field. Since $X$ is a
    scheme of finite type over $\Spec \ZZ$, we conclude that
    $X = \Spec \FF_q \in \mathcal{C} (\ZZ)$.
  \end{proof}
\end{lemma}
