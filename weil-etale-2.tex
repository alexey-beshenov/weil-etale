...
\item[2)] A consequence of \textbf{Conjecture} $\mathbf{B} (X,n)$
  ...
  Here
  $H_\Wc^i (X, \RR (n)) = H_\Wc^i (X, \ZZ (n)) \otimes_\ZZ \RR = H^i (R\Gamma_\Wc (X, \ZZ (n)) \otimes_\ZZ \RR)$.
...
\begin{align*}
  R\Gamma_\fg (X, \QQ (n)) & \dfn R\Gamma_\fg (X, \ZZ (n)) \otimes_\ZZ^\mathbf{L} \QQ = R\Gamma_\fg (X, \ZZ (n)) \otimes_\ZZ \QQ, \\
  R\Gamma_\Wc (X, \QQ (n)) & \dfn R\Gamma_\Wc (X, \ZZ (n)) \otimes_\ZZ^\mathbf{L} \QQ = R\Gamma_\Wc (X, \ZZ (n)) \otimes_\ZZ \QQ.
\end{align*}
Accordingly,
\begin{align*}
  H^i_\fg (X, \QQ (n)) & \dfn H^i (R\Gamma_\fg (X, \QQ (n))) = H^i_\fg (X, \ZZ (n)) \otimes_\ZZ \QQ, \\
  H^i_\Wc (X, \QQ (n)) & \dfn H^i (R\Gamma_\Wc (X, \QQ (n))) = H^i_\Wc (X, \ZZ (n)) \otimes_\ZZ \QQ.
\end{align*}
...
$A (n)$-coefficients. For more details on $G_\RR$-equivariant cohomology, see \cite{Beshenov-Weil-etale-1}. With real coefficients,
...
\begin{align*}
  R\Gamma_\BM (X (\CC), \RR (n)) & \dfn \RHom (R\Gamma_c (X (\CC), \RR (n)), \RR), \\
  R\Gamma_\BM (G_\RR, X (\CC), \RR (n)) & \dfn \RHom (R\Gamma_c (G_\RR, X (\CC), \RR (n)), \RR).
\end{align*}
...
\begin{remark}
  ...
  $$R\Gamma (X_\et, \RR (d-n)) [2d] \to R\Gamma (X_{\CC,\et}, \RR (d-n-1)) [2d].$$
  This probably looks more natural, but we do not impose extra assumptions on $X$
  and work exclusively with complexes $A^c (\bullet)$ defined in terms of
...
\begin{definition}
  ...
  $$Reg_{X,n}\colon R\Gamma (X_\et, \RR^c (n)) \to R\Gamma_\BM (G_\RR, X(\CC), \RR (n)) [1].$$
\end{definition}
...
\begin{remark}
  ...
  $K'$-theory
  $$\ord_{s=n} \zeta (X,s) = \sum_{i \in \ZZ} (-1)^{i+1} \, \dim_\QQ K'_i (X)_{(n)}.$$
...
\begin{remark}[{\cite[Proposition~5.12]{Flach-Morin-2018}}]
  ...
  $$A^{\frac{d-s}{2}}\,\zeta (\overline{X},d-s) = A^{\frac{s}{2}}\,\zeta (\overline{X},s).$$
  Here
  \begin{gather*}
...
\begin{conjecture}
  ...
  $$\lambda (\zeta^* (X,n)^{-1}) \cdot \ZZ = \det_\ZZ R\Gamma_\Wc (X, \ZZ (n)),$$
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    $$\zeta^* (X,n) = \pm \prod_{i \in \ZZ} |H_i^c (X_\ar, \ZZ (n))|^{(-1)^{i+1}}.$$
...
\begin{proposition}
  ...
  \begin{proof}
    ...
    \[ R\Gamma_c (G_\RR, U (\CC), \RR (n)) \to R\Gamma_c (G_\RR, X (\CC), \RR (n)) \to R\Gamma_c (G_\RR, Z (\CC), \RR (n)) \to R\Gamma_c (G_\RR, U (\CC), \RR (n)) [1] \]
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    up to a \emph{unique} isomorphism in the derived category $\mathbf{D} (\ZZ)$,
    and the same argument shows that the induced morphisms of complexes
    \begin{equation}
    ...
    $$H^i (R\Gamma_\fg (-, \ZZ(n))^\wedge_p) \cong H^i_\fg (-, \ZZ(n)) \otimes_\ZZ \ZZ_p.$$
...
\begin{landscape}
  \begin{figure}
    ...
    \caption{Diagram induced by a closed-open decomposition $Z \not\hookrightarrow X \hookleftarrow U$}
...
\begin{landscape}
  \begin{figure}
    ...
    \caption{Isomorphism $R\Gamma_\Wc (\AA^r_X, \ZZ (n)) \cong R\Gamma_\Wc (X, \ZZ (n-r)) [-2r]$ and its splitting after tensoring with $\QQ$}
...
\begin{align*}
  \psi\colon (L,r) \otimes_R (M,s) & \xrightarrow{\cong} (M,s) \otimes_R (L,r), \\
...
\begin{remark}
  ...
  \item[2)] If the cohomology modules $H^i (A^\bullet)$ are perfect, then
    $$\det_R A^\bullet \cong \bigotimes_{i\in \ZZ} (\det_R H^i (A^\bullet))^{(-1)^i}.$$
  \end{enumerate}
\end{remark}
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    \[ \ZZ/m\ZZ [0] \cong \Bigl[
      \mathop{m\ZZ}_{\text{deg.\,}-1} \hookrightarrow \mathop{\ZZ}_{\text{deg.\,}0}
      \Bigr]. \]
    Therefore,
